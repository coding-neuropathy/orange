...
@property (nonatomic) AKSegmentedControl *segmentsBar;
...
- (NSString *)categoryNameAtIndex:(NSUInteger)index {
  NSArray *categoryList = @[@"Recent",@"People", @"Objects", @"Nature", @"Places", @"Symbols",@"Back"];
...
- (NSArray *)imagesForSelectedSegments {
  ...
  dispatch_once(&onceToken, ^{
    ...
    for (AGEmojiKeyboardViewCategoryImage i = AGEmojiKeyboardViewCategoryImageRecent;
         i <= AGEmojiKeyboardViewCategoryImageBack;
         ++i) {
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
  ...
  if (self) {
    // initialize category
    self.backgroundColor = UIColorFromRGB(0x2b2b2b);
    _dataSource = dataSource;

    self.category = [self categoryNameAtIndex:self.defaultSelectedCategory];

    self.segmentsBar = [[AKSegmentedControl alloc] initWithFrame:CGRectMake(0, self.bounds.size.height - 60, CGRectGetWidth(self.bounds), 60)];
    [self setSegmentsBarButton];
    self.segmentsBar.autoresizingMask = UIViewAutoresizingNone;
    self.segmentsBar.tintColor = UIColorFromRGB(0x2b2b2b);
    self.segmentsBar.backgroundColor = UIColorFromRGB(0xffffff);
    ...
    self.segmentsBar.selected = self.defaultSelectedCategory;
...
- (void)setSegmentsBarButton
{
    NSMutableArray * array = [NSMutableArray array];
    for (UIImage * image in self.imagesForSelectedSegments) {
        UIButton *button = [[UIButton alloc] init];
        if (kScreenWidth > 640) {
            [button setImageEdgeInsets:UIEdgeInsetsMake(15.0, 15.0, 15.0, 15.0)];
        }
        else
        {
            [button setImageEdgeInsets:UIEdgeInsetsMake(10.0, 10.0, 10.0, 10.0)];
        }
    ...
    [self.segmentsBar setButtonsArray:array];
    
}

- (void)setSelectedCategoryImageInSegmentControl:(AKSegmentedControl *)segmentsBar AtIndex:(NSInteger)index {
  for (int i=0; i < self.segmentsBar.buttonsArray.count; ++i) {
      [[self.segmentsBar.buttonsArray objectAtIndex:i]setImage:self.imagesForNonSelectedSegments[i] forState:UIControlStateNormal];
  }
  [[self.segmentsBar.buttonsArray objectAtIndex:index]setImage:self.imagesForSelectedSegments[index] forState:UIControlStateNormal];
}

- (void)categoryChangedViaSegmentsBar:(AKSegmentedControl *)sender {
    // recalculate number of pages for new category and recreate emoji pages
    if (sender.selectedIndexes.lastIndex == sender.buttonsArray.count -1) {
        [self emojiKeyBoardViewDidPressBackSpace:nil];
        return;
    }
  // recalculate number of pages for new category and recreate emoji pages
  self.category = [self categoryNameAtIndex:sender.selectedIndexes.lastIndex];
  [self setSelectedCategoryImageInSegmentControl:sender AtIndex:sender.selectedIndexes.lastIndex];
  self.pageControl.currentPage = 0;
  [self setNeedsLayout];
}
...
- (NSMutableArray *)emojiTextsForCategory:(NSString *)category fromIndex:(NSUInteger)start toIndex:(NSUInteger)end {
  ...
  start = ([emojis count] > start)? start : [emojis count];
