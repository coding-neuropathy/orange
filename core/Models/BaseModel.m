...
+ (instancetype)modelFromDictionary:(NSDictionary *)dictionary
{
    ...
    if (keyNameArray && keyNameArray.count > 0) {
        ...
        model = [[BaseModelCenter sharedInstance].modelDictionary valueForKey:centerKey];
        if (model) {
 ...
 */
+ (NSDictionary *)dictionaryFromModel:(BaseModel *)model
{
    ...
    for(i = 0; i < outCount; i++) {
        ...
        if(propName) {
            ...
            // 对于NSURL，转换成NSString，再存入Dictionary，否则Dictionary将不能存入NSUserDefaults。
            id value = [model valueForKey:propertyName];
            if ([value isKindOfClass:[NSURL class]]) {
                [mutableDic setValue:((NSURL *)value).absoluteString forKey:propertyName];
            } else {
                [mutableDic setValue:value forKey:propertyName];
            }
        }
    }
    free(properties);
...
- (void)saveToCenter
{
    ...
    if (subKeyArray && subKeyArray.count > 0) {
        ...
        [[BaseModelCenter sharedInstance].modelDictionary setValue:weakSelf forKey:centerKey];
    } else {
#ifdef DDLogError
...
- (instancetype)initWithCoder:(NSCoder *)decoder
{
    ...
    if (keyNameArray && keyNameArray.count > 0) {
        ...
        BaseModel *model = [[BaseModelCenter sharedInstance].modelDictionary valueForKey:centerKey];
        if (model) {
            return model;
        } else {
            [[BaseModelCenter sharedInstance].modelDictionary setObject:self forKey:centerKey];
        }
    }
...
#pragma mark - BaseModelCenter Class

/**
 *  全局的ModelCenter，用来存储model，实现KVO。
 */
@implementation BaseModelCenter

/**
 *  获取BaseModelCenter单例
 *
 *  @return BaseModelCenter单例
 */
+ (BaseModelCenter *)sharedInstance
{
    static BaseModelCenter *sharedInstance = nil;
